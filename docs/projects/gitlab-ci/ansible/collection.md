...
The following requirements are needed to run these CI/CD jobs.
...
Trigger to create a release. It's only triggered on merge to the `development` and `master` branches. The last stage of this job is to create a git tag which matches the version to be released. The creation of the git tag, starts the remainder of the release cycle. On the `development` branch and only when the job is triggered as it's a manual job--an `alpha` release is created and on the `master` branch, a full release is created.
